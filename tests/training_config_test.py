...
def test_load_config_returns_training_config(
    valid_config: Path, input_shape: TensorShape
):
    config = load_config(valid_config, input_shape)
    assert isinstance(config, TrainingConfig)


def test_load_config_batch_size(valid_config: Path, input_shape: TensorShape):
    config = load_config(valid_config, input_shape)
    assert config.batch_size == 32


def test_load_config_epochs(valid_config: Path, input_shape: TensorShape):
    config = load_config(valid_config, input_shape)
    assert config.epochs == 10


def test_load_config_learning_rate(valid_config: Path, input_shape: TensorShape):
    config = load_config(valid_config, input_shape)
    assert config.learning_rate == pytest.approx(0.001)


def test_load_config_segment_start_defaults_to_zero(
    valid_config: Path, input_shape: TensorShape
):
    config = load_config(valid_config, input_shape)
...
def test_load_config_segment_end(valid_config: Path, input_shape: TensorShape):
    config = load_config(valid_config, input_shape)
    assert config.segment_end == 2


def test_load_config_optimizer_type(valid_config: Path, input_shape: TensorShape):
    config = load_config(valid_config, input_shape)
    assert isinstance(config.optimizer, torch.optim.SGD)


def test_load_config_loss_function_type(valid_config: Path, input_shape: TensorShape):
    config = load_config(valid_config, input_shape)
    assert isinstance(config.loss_function, tnn.CrossEntropyLoss)


def test_load_config_network_is_classificaton_network(
    valid_config: Path, input_shape: TensorShape
):
    config = load_config(valid_config, input_shape)
...
def test_load_config_explicit_start(
    tmp_path: Path, classifier_json: Path, input_shape: TensorShape
):
    content = VALID_TOML_TEMPLATE.format(classifier_path=classifier_json)
    content += "\nstart = 1\n"
    ...
    config = load_config(path, input_shape)
...
def test_load_config_optimizer_variants(
    tmp_path: Path, classifier_json: Path, input_shape: TensorShape, optimizer_name: str
):
    content = VALID_TOML_TEMPLATE.format(classifier_path=classifier_json).replace(
        'name = "SGD"', f'name = "{optimizer_name}"'
    )
    config = load_config(write_toml(tmp_path, content), input_shape)
...
def test_load_config_loss_variants(
    tmp_path: Path, classifier_json: Path, input_shape: TensorShape, loss_name: str
):
    content = VALID_TOML_TEMPLATE.format(classifier_path=classifier_json).replace(
...
def test_missing_batch_size_raises(
    tmp_path: Path, classifier_json: Path, input_shape: TensorShape
):
    content = VALID_TOML_TEMPLATE.format(classifier_path=classifier_json).replace(
        "batch_size = 32\n", ""
...
def test_missing_model_end_raises(
    tmp_path: Path, classifier_json: Path, input_shape: TensorShape
):
    content = VALID_TOML_TEMPLATE.format(classifier_path=classifier_json).replace(
        "end = 2\n", ""
...
def test_missing_learning_rate_raises(
    tmp_path: Path, classifier_json: Path, input_shape: TensorShape
):
    content = VALID_TOML_TEMPLATE.format(classifier_path=classifier_json).replace(
        "learning_rate = 0.001\n", ""
...
def test_missing_classifier_path_raises(tmp_path: Path, input_shape: TensorShape):
...
def test_invalid_loss_function_name_raises(
    tmp_path: Path, classifier_json: Path, input_shape: TensorShape
):
    content = VALID_TOML_TEMPLATE.format(classifier_path=classifier_json).replace(
...
def test_file_not_found_raises(input_shape: TensorShape):
    with pytest.raises(FileNotFoundError):
        load_config(Path("nonexistent.toml"), input_shape)
